...
namespace Calculator
{
    ...
    public class CalcEngine
	{
		...
		public static string CalcEqual()
		{
			...
			else if (firstNumber != 0)
			{
				switch (calcOperation)
				{
					...
					case Operator.eKoren:
						numericAnswer = Math.Sqrt(firstNumber);
						validEquation = true;
						break;

					case Operator.eObrat:
						numericAnswer = 1 / firstNumber;
						validEquation = true;
						break;

					case Operator.eKvadrat:
		...
		public async static Task<string> GetFactorial()
        {
			return await Task.Run(() =>
			{
				string result = Factorial(System.Convert.ToDouble(stringAnswer)).ToString();
				Thread.Sleep(3000);
				return result;
			}
			);
